...
SQLiteFactory.prototype.openDatabase = argsArray(function(args) {
  var dblocation, errorcb, first, okcb, openargs, createFromLocation;
  if (args.length < 1) {
    return null;
  }
  first = args[0];
  ...
  if (!!openargs.createFromLocation) {
    createFromLocation = openargs.createFromLocation;
    if (createFromLocation === 1) {
      openargs.assetFilename = "*default";
    }
    else if (typeof createFromLocation === 'string') {
