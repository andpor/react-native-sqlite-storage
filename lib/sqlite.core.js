...
SQLitePlugin.prototype.attach = function(dbName, dbAlias, success, error) {
  if (this.dbname in this.openDBs) {
    if (txLocks[this.dbname] && txLocks[this.dbname].inProgress) {
    ...
    console.log('ATTACH database ' + dbName + ' to ' + this.dbname + ' with alias ' + dbAlias);
    ...
    plugin.exec("attach",{path: this.dbname, dbName, dbAlias}, mysuccess, myerror);
  } else {
